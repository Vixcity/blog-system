<template>
  ...
  >
    <div class="vicp-wrap">
        ...
        <div class="vicp-crop">
          <div v-show="true" class="vicp-crop-left">
            ...
            <div v-if="isGif" class="vicp-range">
              ...
              <i class="vicp-icon5" @mousedown="startFpsSub" @mouseup="endFpsSub" @mouseout="endFpsSub"></i>
              <i class="vicp-icon6" @mousedown="startFpsAdd" @mouseup="endFpsAdd" @mouseout="endFpsAdd"></i>
            </div>
          ...
          <div v-show="true" class="vicp-crop-right">
            <div class="vicp-preview">
              <div v-if="!noSquare" class="vicp-preview-item">
                <img id="get_gif" :src="createImgUrl" :style="previewStyle">
      ...
      <canvas v-show="false" id="canvas_use" ref="canvas" :width="width" :height="height"></canvas>
    </div>
    <div
        style="display: fiexd; left: 0; right: 0;top: 0;bottom: 0;width: 100%;height: 100%"
        :style="{'z-index': fullscreenLoading ? 10001 : 0}"
...
export default {
  ...
  data() {
    ...
    const tempImgFormat = allowImgFormat.indexOf(imgFormat) === -1 ? 'png' : imgFormat
    const lang = {
      hint: '点击，或拖动图片至此处(支持gif哦)',
      loading: '正在上传……',
  ...
  methods: {
    ...
    pre_load_gif(gif_source) {
      ...
      document.getElementById('canvas_use').appendChild(gifImg)
      // 新建gif实例
      ...
      rub.load(() => {
        ...
        for (let i = 1; i <= rub.get_length(); i++) {
          ...
          const cur_file = this.convertCanvasToImage(rub.get_canvas(), gif_source.name.replace('.gif', '') + `-${i}`)
          img_list.push(cur_file)
        }
    ...
    prepareUpload() {
      ...
      if (this.isGif) {
        ...
        this.$emit('filesend', this.createImgUrl, this.data2blob(createImgUrl, mime), field + '.' + imgFormat)
