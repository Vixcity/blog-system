<template>
  <vuescroll>
    <div class="categories">
      <contentBanner :imgSrc="bannerSrc" :title="tag" />
      <div class="container">
        <div class="page">
          <div class="category-lists">
            <div class="category-title is-center">
              {{ tag }} -
              <span class="category-amount">{{ tagList.length }}</span>
            </div>
            <div>
              <hr>
              <ul class="category-list">
                <li v-for="(item,index) in tagList" :key="index" class="category-list-item">
...
export default {
  ...
  methods: {
    ready() {
      ...
      this.$api.getTagsList({tag:this.tag}).then(r=>{
        this.tagList = r.data
      })
